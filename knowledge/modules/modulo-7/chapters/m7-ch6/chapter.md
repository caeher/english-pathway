---
id: m7-ch6
moduleId: modulo-7
number: 6
title: Final Review and Comprehensive Practice
subtitle: Demonstrate everything you have learned
icon: 🏆
color: "#a855f7"
objectives:
  - Review all the topics covered in the course modules
  - Combine tenses, modals, conditionals, passive voice and more
  - Solve mixed exercises of advanced difficulty
xpReward: 200
---

## Final Review — Final Review and Comprehensive Practice

### What are you going to review?

This chapter integrates **everything** what was learned in the course:

| Area | Key Topics |
|------|-----------|
| **Tenses** | Present/Past Simple, Continuous, Perfect, Future |
| **Modals** | can, could, must, should, may, might |
| **Conditionals** | 1st, 2nd, 3rd conditional |
| **Passive Voice** | be + past participle in all tenses |
| **Reported Speech** | said/told + backshift |
| **Relative Clauses** | who, which, that, where, whose |
| **Connectors** | however, therefore, although, despite, etc. |
| **Phrasal Verbs** | look up, give up, turn on/off, etc. |
| **Idioms** | a piece of cake, under the weather, etc. |
| **Common Mistakes** | S-V agreement, articles, false friends |

---

### Review strategy

1. **Read the theory** if you don't remember a topic.
2. **Do the activities** in this chapter — they are mixed.
3. **Go back** to previous modules if you need to reinforce a point.

---

### Visual summary by module

| Module | Focus |
|--------|-------|
| 8 | Fundamentals: alphabet, greetings, pronouns, articles |
| 9 | Present & Past Simple, verbs, frequency |
| 10 | Continuous tenses, future, basic modals |
| 11 | Comparatives, superlatives, modals, prepositions |
| 12 | Perfect tenses, passive voice |
| 13 | Conditionals, reported speech, phrasal verbs |
| 14 | Relative clauses, connectors, idioms, advanced structures |

---

### Congratulations!
If you complete all the activities in this chapter, you will have demonstrated that you have mastered English grammar from basic to advanced levels. 🎉
