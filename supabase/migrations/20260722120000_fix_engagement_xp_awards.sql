-- Make the XP ledger consistent: one activity award is idempotent and achievement
-- rewards are credited exactly once to both lifetime and daily XP totals.
...
BEGIN
  ...
  -- These limits mirror the API contract and prevent direct RPC calls from
  -- corrupting the engagement counters.
  ...
  VALUES (v_user_id, v_activity_xp, 1, 1, p_local_date)
  ON CONFLICT (user_id) DO UPDATE SET
    total_xp = public.user_engagement.total_xp + v_activity_xp,
    current_streak = CASE
  ...
  VALUES (v_user_id, p_local_date, p_minutes, v_activity_xp, 1, p_minutes >= v_daily_goal)
  ...
  -- Re-evaluate after every award so an achievement reward that crosses an XP
  -- threshold can unlock its follow-up achievement in the same completion.
  LOOP
  ...
  RETURN jsonb_build_object(
    'xpAwarded', v_activity_xp + v_achievement_xp,
    'activityXpAwarded', v_activity_xp,
