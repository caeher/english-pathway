...
export default async function CurriculumPage() {
  ...
  const resume = getLearningTarget(modules, snapshot)
  const continuation = getLearningContinuation({
    dueReviews: await getDueCount(supabase, user.id),
    resume,
  ...
  return (
    <PageContainer>
      ...
      <p className="mt-4 max-w-2xl text-lg leading-relaxed text-(--text-secondary)">Follow the structured pathway chapter by chapter, or open Learn anytime for guided conversation.</p>
      <div className="mt-8 flex flex-wrap items-center gap-3">
        ...
        <span className="inline-flex items-center gap-2 rounded-xl bg-(--success-soft) px-4 py-2 text-sm font-bold text-(--success)">
      ...
      <p className="mt-4 text-sm text-(--text-secondary)">{continuation.description}</p>
          ...
          return (
            <Link key={curriculumModule.id} href={curriculumModuleHref(curriculumModule.id)} className="group rounded-2xl border border-(--border-primary) bg-(--bg-card) p-6 no-underline transition-transform hover:-translate-y-1">
              ...
              <div className="mt-5 flex items-center justify-between text-sm font-bold text-(--accent)">
                ...
                <span>{moduleProgress.completedChapters}/{moduleProgress.totalChapters}</span>
                <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" />
              </div>
              {nextChapter && <p className="mt-3 text-xs text-(--text-muted)">Next: {curriculumModule.chapters.find((chapter) => chapter.id === nextChapter.chapterId)?.title}</p>}
            </Link>
          )
        })}
      </div>
