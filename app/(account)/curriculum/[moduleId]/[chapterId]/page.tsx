...
import CurriculumExerciseRunner from '@/components/curriculum/CurriculumExerciseRunner'
  ...
  return (
    <PageContainer id="chapter-top">
      ...
      <header className="mt-7 rounded-3xl border border-(--border-primary) bg-(--bg-card) p-7 sm:p-10">
        ...
        <div className="mt-7 flex flex-wrap gap-3 text-sm font-bold text-(--text-secondary)">
          <span className="inline-flex items-center gap-2 rounded-lg bg-(--bg-tertiary) px-3 py-2"><ListChecks className="h-4 w-4 text-(--accent)" aria-hidden="true" /> {progress.totalRequiredActivities} required exercises</span>
          <span className="inline-flex items-center gap-2 rounded-lg bg-(--bg-tertiary) px-3 py-2"><Headphones className="h-4 w-4 text-(--accent)" aria-hidden="true" /> Use speaker buttons for highlighted phrases</span>
        </div>
        <div className="mt-5 h-2 overflow-hidden rounded-full bg-(--bg-tertiary)" role="progressbar" aria-label="Chapter exercise progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.completionPercent}><div className="h-full rounded-full bg-(--accent)" style={{ width: `${progress.completionPercent}%` }} /></div>
        <p className="mt-2 text-sm text-(--text-secondary)">{progress.passedRequiredActivities} of {progress.totalRequiredActivities} required exercises passed</p>
        <a href="#practice" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-(--accent) px-4 py-2 text-sm font-bold text-white no-underline hover:bg-(--accent-hover)">Start practice <ArrowRight className="h-4 w-4" /></a>
      </header>

      {chapter.objectives.length > 0 && (
        <section className="mt-8 rounded-2xl border border-(--border-primary) bg-(--bg-card) p-6">
          <h2 className="flex items-center gap-2 font-display text-xl font-black text-(--text-primary)"><BookOpen className="h-5 w-5 text-(--accent)" aria-hidden="true" /> What you will learn</h2>
          <ul className="mt-4 space-y-2 text-(--text-secondary)">{chapter.objectives.map((objective) => <li key={objective}>• {objective}</li>)}</ul>
        </section>
      )}
      {headings.length > 1 && (
        <nav aria-label="Chapter contents" className="mt-8 rounded-2xl border border-(--border-primary) bg-(--bg-card) p-6">
          <h2 className="font-display text-lg font-black text-(--text-primary)">In this chapter</h2>
          <ol className="mt-3 space-y-2 text-sm">{headings.map((heading) => <li key={heading.id} className={heading.level === 3 ? 'pl-4' : undefined}><a href={`#${heading.id}`} className="font-bold text-(--accent) underline-offset-4 hover:underline">{heading.text}</a></li>)}</ol>
        </nav>
      )}
      ...
      <CurriculumExerciseRunner
        ...
        nextChapter={next ? { moduleId: next.module.id, chapterId: next.chapter.id, title: next.chapter.title } : null}
      />
