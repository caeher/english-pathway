      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {modules.map((module) => (
          <Link key={module.id} href={curriculumModuleHref(module.id)} className="group rounded-2xl border border-(--border-primary) bg-(--bg-card) p-6 no-underline transition-transform hover:-translate-y-1">
            ...
            <p className="mt-5 flex items-center gap-2 text-sm font-bold text-(--accent)"><Layers className="h-4 w-4" aria-hidden="true" /> {module.chapters.length} chapters <ArrowRight className="ml-auto h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></p>
          </Link>
        ))}
      </div>
    </div>
