...
describe('ActivityRenderer coverage', () => {
  ...
  it('delegates post-activity feedback to the learn shell', () => {
    ...
    expect(renderer).toContain('handleSkip')
    expect(renderer).toContain('onOutcome')
    expect(renderer).toContain('createSkippedActivityOutcome')
    expect(renderer).toContain('createClosedActivityOutcome')
  })
})
