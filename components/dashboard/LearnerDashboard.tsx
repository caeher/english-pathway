...
export default function LearnerDashboard({ data }: { data: DashboardData }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header>
        <p className="text-sm font-bold uppercase tracking-wide text-(--accent)">Your learning hub</p>
        <h1 className="mt-1 font-display text-3xl font-black text-(--text-primary)">Welcome back, {firstName}</h1>
        <p className="mt-2 text-(--text-secondary)">Keep your momentum going with a focused practice session.</p>
      </header>

      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-(--border-primary) bg-(--bg-card) p-5">
          <div className="flex items-center justify-between"><Sparkles className="h-5 w-5 text-(--reward)" /><span className="text-xs font-bold text-(--text-muted)">LEVEL {level.level}</span></div>
        ...
        <div className="rounded-2xl border border-(--border-primary) bg-(--bg-card) p-5">
          <div className="flex items-center justify-between"><Flame className="h-5 w-5 text-(--accent)" /><span className="text-xs font-bold text-(--text-muted)">BEST {data.engagement?.longest_streak ?? 0}</span></div>
        ...
        <div className="rounded-2xl border border-(--border-primary) bg-(--bg-card) p-5">
          <div className="flex items-center justify-between"><Trophy className="h-5 w-5 text-(--secondary)" /><span className="text-xs font-bold text-(--text-muted)">CHAPTERS</span></div>
        ...
        <div className="rounded-2xl border border-(--border-primary) bg-(--bg-card) p-5">
          <div className="flex items-center justify-between"><RotateCcw className="h-5 w-5 text-(--accent)" /><span className="text-xs font-bold text-(--text-muted)">SRS</span></div>
      ...
      <section className="rounded-2xl border border-(--border-primary) bg-(--bg-card) p-6">
        <div className="flex items-center justify-between gap-4"><div><h2 className="font-display text-xl font-black text-(--text-primary)">Recent activity</h2><p className="mt-1 text-sm text-(--text-secondary)">Your latest practice results.</p></div><Link href="/review" className="inline-flex items-center gap-1 text-sm font-bold text-(--accent) no-underline hover:underline">Review {data.dueReviews > 0 && `(${data.dueReviews})`} <ArrowRight className="h-4 w-4" /></Link></div>
